...
def connect_to(host,port,type):
    ...
    while 42:
        ...
        try:
            ...
            # this code may be outdated now, exc doesn't have errno and it should wait for the timeout for the connection to be available
...
remote= ''
remote_type= ''
if args[0].find('emulator:') == 0:
    remote= args[0][9:]
    em_remote= True
    remote_type= 'EMULATOR'
else:
    em_remote= False
if args[0].find('reader:') == 0:
    remote= args[0][7:]
    rd_remote= True
    remote_type= 'READER'
    emulator= card
